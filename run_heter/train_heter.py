import sys
sys.path.append("..")
from run_heter import utils2
import seaborn as sb
import dgl
import torch
from run_heter.model import GNNStructEncoder
...
# Training
def train(g, feats, lr, epoch, device, encoder, lambda_loss, hidden_dim, sample_size=10,moving_average_decay=0.0):
    ...
    GNNModel = GNNStructEncoder(in_dim, hidden_dim, 2, sample_size, GNN_name=encoder, lambda_loss=lambda_loss, tau=args.tau,  moving_average_decay=moving_average_decay, num_MLP=args.num_MLP)
    GNNModel.to(device)
...
def train_new_datasets(dataset_str, epoch_num = 10, lr = 5e-6, encoder = "GCN", lambda_loss=1, sample_size=10, hidden_dim=None,moving_average_decay=0.0):
    ...
    node_embeddings, loss = train(g, node_features, lr=lr, epoch=epoch_num, device=device, encoder=encoder,
...
if __name__ == '__main__':
    ...
    train_new_datasets(dataset_str=dataset_str, lr=args.lr, epoch_num=args.epoch_num, lambda_loss=args.lambda_loss, encoder="GCN", sample_size=args.sample_size, hidden_dim=args.dimension,moving_average_decay=args.moving_average_decay)
