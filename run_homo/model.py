...
class Model(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, out_dim, num_layers, temp, use_mlp=False, moving_average_decay=1.0, num_MLP=1, lambda_loss=1):
    ...
    def pos_score(self, graph, h, trans_feature):
        ...
        graph.ndata['z'] = F.normalize(trans_feature, dim=-1)
        if self.num_MLP != 0:
            graph.ndata['q'] = F.normalize(self.projector(h))
        else:
            graph.ndata['q'] = F.normalize(h)
